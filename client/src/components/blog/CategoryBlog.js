...
import { getPostsCategory } from "../../actions/post";
...
const CategoryBlog = ({
  match,
  getPostsCategory,
  post: { categoryPosts, loadingCategoryPosts },
}) => {
  useEffect(() => {
    getPostsCategory(match.params.category);
  }, [match.params.category, getPostsCategory]);
  return (
    <Fragment>
      <TitleHeader title="Blog Category" pageTitle={match.params.category} />
...
CategoryBlog.propTypes = {
  ...
  getPostsCategory: PropTypes.func.isRequired,
};

export default connect(mapStateToProps, { getPostsCategory })(CategoryBlog);
