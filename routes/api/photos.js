...
router.post('/', authPhotographer, async (req, res) => {
	...
	try {
		...
		const ajdImage = photoImage.split('&#x2F;').join('/');
		...
		crypto.randomBytes(16, (err, buf) => {
			...
			fs.writeFile(
				path.join(__dirname, `../../client/public/uploads/${filename}.${ext}`),
				buffer,
				async (err) => {
					if (err) res.status(400).json('Image not upload');
					const newPhoto = new Photo({
						photoImage: `/uploads/${filename}.${ext}`,
						title,
						category,
						keywords,
						name: user.name,
...
router.delete('/:id', authPhotographer, async (req, res) => {
	try {
		...
		fs.unlink(
			path.join(__dirname, `../../client/public${photo.photoImage}`),
			(err) => {
